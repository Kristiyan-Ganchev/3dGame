                ...
                case CharacterState.Default:
                    {
                        ...
                        if (Motor.GroundingStatus.IsStableOnGround)
                        {
                            ...
                            //handle dodging
                            if (_dodgeRequested && canDodge)
                            {
                                if (reorientedInput == Vector3.zero) inputRight = Vector3.Cross(-transform.forward, Motor.CharacterUp);
                                targetMovementVelocity = Vector3.Cross(effectiveGroundNormal, inputRight).normalized * dodgeSpeed;
                                currentVelocity = Vector3.Lerp(currentVelocity, targetMovementVelocity, 1f - Mathf.Exp(-StableMovementSharpness * deltaTime / dodgeDuration));
                                _dodgeRequested = false;
                                canDodge = false;
                                Invoke(nameof(ResetDodge), dodgeCooldown);

                                animMan.PlayTargetAnimation("dodgeBaked", true);
                                return;
                            }
                        ...
                        if (_jumpRequested)
                        {
                            ...
                            if (!_jumpConsumed && ((AllowJumpingWhenSliding ? Motor.GroundingStatus.FoundAnyGround : Motor.GroundingStatus.IsStableOnGround) || _timeSinceLastAbleToJump <= JumpPostGroundingGraceTime))
                            {
                                ...
                                if (!_sprintRequested)
                                {
                                    animMan.PlayTargetAnimation("JumpBaked",true);
                                }
                                else
                                {
                                    animMan.PlayTargetAnimation("sprintJumpBaked", true);
                                }
